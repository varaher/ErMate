...
export const EXAM_DEFAULTS = NORMAL_EXAM_TEMPLATES;
...
export const detectNormalcyPhrases = detectClinicalNormalcy;
...
export function applyExaminationDefaults(
  ...
  const { abcdeNormal, systemicNormal, sectionNormals } = detectNormalcyPhrases(rawText);
  ...
  for (const [field, defaultValue] of Object.entries(EXAM_DEFAULTS)) {
    ...
    if (!isEmpty) {
      ...
      // A model may return only "normal" / "fine" / "NAD". Convert that

      // label to the approved deterministic template for this section.

      // Specific findings are preserved exactly and are never expanded.

      if (field !== "psychologicalAssessment" && isNormalOnlySectionValue(result[field])) {
    ...
    const trigger = PRIMARY_SURVEY_FIELDS.has(field)

      ? abcdeNormal

      : (systemicNormal || (section ? sectionNormals[section] : false));
...
CLINICAL COMPLETENESS REQUIREMENT — extract all stated clinical fields into the canonical flat JSON schema below
