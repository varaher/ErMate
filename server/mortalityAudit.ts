...
export async function generateMortalityAudit(
  ...
  if (process.env.OPENAI_API_KEY && process.env.OPENAI_API_KEY.trim() !== "") {
    try {
     ...
     // 3. Fallback Model: Gemini Pro ONLY — per locked model matrix, M&M
  // Audit is explicitly "Never Gemini Flash". This is a confidential,
  // HOD-only, medico-legal document that determines preventability
  // classification — it must never be silently generated by a
  // quick-correction-tier model. No Flash candidates permitted here,
  // matching the hard-refuse pattern in handover.ts.
  if (process.env.GEMINI_API_KEY && process.env.GEMINI_API_KEY.trim() !== "") {
    try {
      console.log("[MortalityAudit] Attempting fallback model: gemini-1.5-pro...");
      ...
      console.warn("[MortalityAudit] Gemini Pro fallback failed:", err?.message || err);
    }
  }
