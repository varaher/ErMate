...
const roundsMatch = /\{roundsUserMessage\.trim\(\) === "" \? \([\s\S]*?<VoiceRecorder[\s\S]*?\/>\n\s*\) : \(\n\s*(<button[\s\S]*?onClick=\{\(\) => handleRoundsChatSend\(\)\}[\s\S]*?<\/button>)\n\s*\)\}/;

code = code.replace(roundsMatch, (match, p1) => {
  return `<button
                          type="button"
