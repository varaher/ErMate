...
if (sIdx !== -1) {
  let sub = content.substring(sIdx);
  const nextPIdx = sub.indexOf('// SECTION 4 - INVESTIGATIONS');
  if (nextPIdx !== -1) {
  ...
  children.push(new Paragraph(""));
  `;
     content = content.substring(0, sIdx) + newVbg + sub.substring(nextPIdx);
