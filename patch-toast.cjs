...
const oldCode = `      setMessages(prev => prev.map(m => m.id === msgId ? { ...m, extractionApplied: true } : m));`;

const newCode = `      setMessages(prev => prev.map(m => m.id === msgId ? { ...m, extractionApplied: true } : m));
      // Show feedback toast
...
console.log("Patched toast again");
