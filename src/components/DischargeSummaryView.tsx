...
**MEMORIAL MULTISPECIALTY HOSPITAL & RESEARCH CENTER**
**Clinical Department of Emergency Medicine & Level 1 Trauma Services**
--------------------------------------------------
...
<h4><strong>MEMORIAL MULTISPECIALTY HOSPITAL & RESEARCH CENTER</strong></h4>
<h5><strong>Clinical Department of Emergency Medicine & Level 1 Trauma Services</strong></h5>
<hr/>
...
`;
  ...
  const handleDownloadDischargeSummaryWord = () => {
    ...
    const htmlContent = `
      ...
      <head>
        <title>Discharge Summary - ${currentCase.patient.name}</title>
        <!--[if gte mso 9]>
        <xml>
          <w:WordDocument>
            <w:View>Print</w:View>
        ...
        <style>
          ...
          h1 { color: #1e293b; border-bottom: 2px solid #e2e8f0; padding-bottom: 8px; font-size: 18pt; margin-top: 0; text-align: center; }
          h2 { color: #0f172a; margin-top: 24px; font-size: 13pt; border-bottom: 1px solid #cbd5e1; padding-bottom: 4px; font-weight: bold; }
          p { margin: 8px 0; font-size: 10.5pt; }
          .footer { font-size: 8pt; color: #64748b; border-top: 1px solid #e2e8f0; padding-top: 10px; margin-top: 40px; }
        </style>
      </head>
      <body>
        <h1>Discharge Summary</h1>
        <div style="white-space: pre-wrap; font-family: 'Segoe UI', Arial, sans-serif; font-size: 11pt; line-height: 1.6;">
          ${text.replace(/\n/g, "<br/>")}
        </div>
        <div class="footer">
          Generated by ErMate on ${new Date().toLocaleString()}
        </div>
      </body>
      </html>
    `;
    ...
    const link = document.createElement("a");
    link.href = url;
    link.download = `Discharge_Summary_${currentCase.patient.name.replace(/\s+/g, "_")}_${currentCase.id}.doc`;
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    URL.revokeObjectURL(url);
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6" id="discharge-summary-container">
        ...
        <div className="flex gap-2">
          ...
          <div className="relative inline-block text-left" id="download-dropdown-discharge-container">
            <button
              ...
              className="inline-flex items-center gap-1.5 px-3 py-2 bg-slate-50 hover:bg-slate-100 text-slate-700 text-xs font-bold rounded-lg transition-all dark:bg-slate-800 dark:text-slate-300 border border-slate-200 dark:border-slate-800"
            ...
            {downloadMenuOpen && (
              <div className="absolute right-0 mt-1 w-48 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg shadow-xl z-50 text-xs py-1">
                <button
                  onClick={() => {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div className="lg:col-span-7 bg-white text-slate-900 border border-slate-300 rounded-2xl shadow-lg overflow-hidden flex flex-col print:border-0 print:shadow-none print:rounded-none">
          ...
          <div className="p-8 md:p-10 font-sans leading-relaxed text-[11px] text-slate-900 bg-white space-y-5 select-text max-w-full print:p-0" id="print-sheet-content">
