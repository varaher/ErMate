...
function ReasoningBubble({ message }: { message: ScribeChatMessage }) {
  ...
  return (
    <div className="flex items-start gap-2 max-w-[90%] animate-fade-in">
      <BookOpen size={16} className="text-indigo-400 mt-0.5 flex-shrink-0" />
      <div className="bg-slate-900 border border-slate-800 rounded-2xl rounded-tl-sm px-4 py-3 space-y-3 shadow-md">
        {message.content && <p className="text-xs text-slate-200 leading-relaxed font-sans">{message.content}</p>}

        {differentials.length > 0 && (
          <div className="bg-slate-950/60 border border-slate-800/80 rounded-xl p-2.5">
            <p className="text-[10px] font-black tracking-widest text-indigo-400 uppercase mb-1.5 flex items-center gap-1">
...
export default function ScribeClinicalChat({
  ...
  if (isHistoryLoading) {
    return (
      <div className="flex flex-col h-[75vh] bg-slate-950 items-center justify-center rounded-2xl border border-slate-800">
        <div className="w-7 h-7 border-2 border-indigo-500 border-t-transparent rounded-full animate-spin mb-2" />
        <p className="text-xs text-slate-400 font-medium">Loading patient chat history...</p>
      </div>
    );
  }

  return (
    <div className="flex flex-col h-[75vh] bg-slate-950 rounded-2xl border border-slate-800 overflow-hidden shadow-xl animate-fade-in">
      {/* Header Bar */}
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-3.5">
        {messages.length === 0 && (
          <div className="text-center py-12 px-6 bg-slate-900/50 border border-slate-800/80 rounded-2xl my-4">
            <Sparkles className="w-8 h-8 text-indigo-400 mx-auto mb-2 opacity-80" />
            <h3 className="text-sm font-bold text-white mb-1">ErMate Scribe & Chat Ready</h3>
        ...
        {isSending && (
          <div className="flex items-center gap-2 text-indigo-400 text-xs py-2 px-3 bg-slate-900/80 border border-slate-800 rounded-xl w-fit">
      ...
      {/* Input Bar */}
      <div className="border-t border-slate-800 bg-slate-900 px-3 py-3 flex items-center gap-2">
        <input
          value={inputText}
          onChange={e => setInputText(e.target.value)}
          onKeyDown={e => e.key === "Enter" && !e.shiftKey && handleSend(inputText)}
          placeholder="Dictate, type notes, or ask a clinical question..."
          className="flex-1 bg-slate-950 border border-slate-700 rounded-full px-4 py-2 text-xs text-white placeholder:text-slate-500 focus:outline-none focus:border-indigo-500"
          disabled={isSending}
        />
