...
const oldCode = `      setMessages(prev => prev.map(m => m.id === msgId ? { ...m, dischargeApplied: true } : m));`;

const newCode = `      setMessages(prev => prev.map(m => m.id === msgId ? { ...m, dischargeApplied: true } : m));
      // Show feedback toast
...
console.log("Patched discharge toast");
